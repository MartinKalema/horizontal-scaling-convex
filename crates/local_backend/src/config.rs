...
pub struct LocalConfig {
    ...
    /// Previous deployment secret accepted only for cluster-internal gRPC auth
    /// during rolling credential rotation. Nodes still send the current
    /// instance secret-derived token.
    #[clap(long, env = "CLUSTER_GRPC_PREVIOUS_INSTANCE_SECRET")]
    pub cluster_grpc_previous_instance_secret: Option<String>,

    /// File containing the previous deployment secret accepted only for
    /// cluster-internal gRPC auth during rolling credential rotation.
    #[clap(long, env = "CLUSTER_GRPC_PREVIOUS_INSTANCE_SECRET_PATH")]
    pub cluster_grpc_previous_instance_secret_path: Option<String>,
...
impl LocalConfig {
    ...
    pub fn previous_cluster_grpc_secret(&self) -> anyhow::Result<Option<InstanceSecret>> {
        let secret = match (
            &self.cluster_grpc_previous_instance_secret,
            &self.cluster_grpc_previous_instance_secret_path,
        ) {
            (Some(_), Some(_)) => anyhow::bail!(
                "Set only one of CLUSTER_GRPC_PREVIOUS_INSTANCE_SECRET or \
            ...
            (None, Some(path)) => Some(
                fs::read_to_string(path)
                    ...
                    .map_err(|err| {
                        anyhow::anyhow!(
                            "Failed to read CLUSTER_GRPC_PREVIOUS_INSTANCE_SECRET_PATH: {err}"
                        )
                    })?,
            ),
            (None, None) => None,
        ...
        secret
            .map(|secret| InstanceSecret::try_from(secret.as_str()))
            .transpose()
    }
