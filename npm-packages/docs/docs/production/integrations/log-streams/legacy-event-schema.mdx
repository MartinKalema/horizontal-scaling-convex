...
- `_topic = "_audit_log"`
- `_timestamp` = Unix epoch timestamp in milliseconds
- `action = "create_environment_variable" | "update_environment_variable" | "delete_environment_variable" | "replace_environment_variable" | "deploy" | "push_config" | "build_indexes" | "change_deployment_state"`
- `actionMetadata` = object whose fields depends on the value of the `action`
  field.

Example legacy `push_config` audit log:
